...
def login_after():
    ...
    if(s):
        ...
        return "Invalid", status.HTTP_404_NOT_FOUND
...
def register():
    ...
    us=y["username"]
    ps=y["password"]
    cps=y["confirmPassword"]
    em=y["emailId"]
    bday=y["birthDate"]
    up=y["userType"].lstrip("0").replace(" 0", " ")
    print(us,ps,cps,em,bday,up)
    session=g.session()
    s=''
    regex = '^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$'
    print("bday is",bday)
...
def showfriend():
    ...
    for n in nodes2:
        total={}
        total['username']=n['users']
        total['role']=n['role']
        if n['users'] in follow:
            total['status']=1
        else:
            total['status']=0
        lis.append(total)
    lis[:] = [d for d in lis if d.get('username') != usname]
    print(lis)
...
def purchases():
    l=[]
    usname=request.args.get('usname')
    q5="match(u:user{username:'"+usname+"'})-[f:follows]->(n:user) match(n:user)-[o:owns]->(p:product) return n.username,p.price,p.name,p.category,ID(p)"
    nodes=session.run(q5)
    for n in nodes:
        p=Product(n['p.name'],n['p.price'],n['n.username'],n['ID(p)'],n['p.category'])
        l.append(p)
    return json.dumps([ob.__dict__ for ob in l])
